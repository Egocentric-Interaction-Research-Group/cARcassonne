            ...
            if (target != null)
            {
                ...
                if (rigidBody != null)
                {
                    ...
                    if (profile != null && profile.FocusIndividualCompoundCollider == false)
                    {
                        ...
                        EditorGUILayout.HelpBox($"When using Bounding Box in combination with Rigidbody 'Focus Individual Compound Collider' must be enabled in Input Profile.", MessageType.Warning);

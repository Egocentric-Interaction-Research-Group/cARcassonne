...
public static class BoardObservation
{
    ...
    public static Dictionary<Vector2Int, int> BuildMeepleMap(AIWrapper wrapper)
    {
        ...
        List<MeepleScript> allMeeps = wrapper.state.Meeples.All;
        Dictionary<Vector2Int, int> mappedMeeps = new Dictionary<Vector2Int, int>(allMeeps.Count);
        foreach (MeepleScript meep in allMeeps)
        {
            if (meep.free)
                continue;

            int meepleData = 0x0;
            meepleData |= meep.player.getID() & bitMask3;        // Insert 3-bit player id for meeple. Must be between 0-7.
